...
export class JwtInterceptor implements HttpInterceptor {
  ...
  private addToken(req: HttpRequest<any>) {
    if (this._authenticationService.currentAccessToken) {
      return req.clone({
        headers: new HttpHeaders({
          Authorization: `Bearer ${this._authenticationService.currentAccessToken}`
        })
      });
    } else {
      return req;
  ...
  private async handle400Error(error: HttpErrorResponse) {
  ...
  private handle401Error(request: HttpRequest < any >, next: HttpHandler): Observable < any > {
    ...
    if(!this.isRefreshingToken) {
      ...
      this._authenticationService.currentAccessToken = null;

      // First, get a new access token
      return this._authenticationService.getNewAccessToken().pipe(
        switchMap((token: any) => {
          if (token) {
            // Store the new token
            const accessToken = token.accessToken;
            return this._authenticationService.storeAccessToken(accessToken).pipe(
              switchMap(_ => {
